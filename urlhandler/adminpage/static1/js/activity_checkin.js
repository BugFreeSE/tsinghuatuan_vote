...
function append_tickets_table(uid) {
    ...
    ++tickets_count;
    $('#tickets-table-body').prepend('<tr class="warning"><td class="ticket-no">' + tickets_count + '</td><td class="ticket-status"><img class="ticket-status-img" src="/static1/img/checking.gif"></td><td class="ticket-uid">' + uid + '</td><td class="ticket-stuid"></td><td class="ticket-time">' + new Date().toLocaleTimeString() + '</td><td class="ticket-msg"></td>');
}
...
function update_tickets_table(data) {
    ...
    $(c[1]).children().attr('src', '/static1/img/' + ticket_result_img_map[data.result]);
    $(c[3]).text(data.stuid);
